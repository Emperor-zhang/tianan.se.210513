    ...
    formatDate() {
      ...
      if (
        ...
        formatDateArray = this.dateTime.map((item, index) => {
          return this[names[index] + "s"][item] < 10
            ? addZero(this[names[index] + "s"][item])
        ...
        formatDateArray = this.dateTime.map((item, index) => {
          if (index > 2) {
            return this[names[index - 3] + "s"][item] < 10
              ? addZero(this[names[index - 3] + "s"][item])
              : this[names[index - 3] + "s"][item];
          } else {
            return this[name1[index] + "s"][item] < 10
              ? addZero(this[name1[index] + "s"][item])
              : this[name1[index] + "s"][item];
          }
        });
        dateString =
      ...
      this.$emit("change", { a: dateString, b: formatDateArray });
    },
    dateTimePickerChange(e) {
      ...
      if (
        ...
        if (columns[0] != this.dateTime[0]) {
          this.days = this.initTimeData(
            getOneMonthDays(
              this.years[this.dateTime[0]],
              this.months[this.dateTime[1]]
            ),
            1
          );
