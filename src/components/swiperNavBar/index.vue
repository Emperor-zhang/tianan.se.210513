<template>
  ...
  >
    <view
      ...
      :key="index"
      :style="{
        width: currentSwiperWidth,
        lineHeight: currentSwiperHeight + 'rpx',
      }"
      :id="'tab-' + index"
      @tap="CurrentTab(index, item.imgName)"
    >
      <!--导航标题-->
      <text
        :style="{
          color: swiperTabIdx == index ? swiperCurrentColor : swiperColor,
          fontSize: swiperCurrentSize,
        }"
      >
        {{ item.imgName }}
      </text>
      ...
      <view class="swiperLine" v-show="currentSwiperLineShow">
        <view
          ...
          :class="[
            swiperTabIdx == index && currentSwiperLineAnimatie
              ? 'currentLine'
              : '',
          ]"
          :style="{
